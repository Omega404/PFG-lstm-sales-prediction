...
# Configuración multi-temporal AJUSTADA AL DATASET
TEMPORAL_CONFIGS = {
    'short': {
        'name': 'Corto Plazo',
        'window_size': 28,      # 4 semanas de histórico (reducido de 42)
        'forecast_horizon': 14, # Predice 2 semanas
        'epochs': 80,
        'description': 'Decisiones tácticas inmediatas (próximas 2 semanas)'
    },
    'medium': {
        'name': 'Medio Plazo',
        'window_size': 90,      # 3 meses de histórico (reducido de 180)
        'forecast_horizon': 30, # Predice 1 mes
        'epochs': 100,
        'description': 'Planificación mensual y reabastecimiento'
    },
    'long': {
        'name': 'Largo Plazo',
        'window_size': 180,     # 6 meses de histórico (reducido de 365)
        'forecast_horizon': 60, # Predice 2 meses (reducido de 90)
        'epochs': 120,
        'description': 'Estrategia trimestral y presupuesto'
    }
}
...
class TemporalProductAnalyzer:
    ...
    def load_and_prepare_data(self, stock_code):
        ...
        # DEBUG: Mostrar primeros códigos
        print(f"   Primeros códigos en Excel: {df['StockCode'].head(3).tolist()}")
        
        # Convertir StockCode a string y limpiar
        df['StockCode'] = df['StockCode'].astype(str).str.strip()
        stock_code = str(stock_code).strip()
        
        # DEBUG: Verificar si existe
        existe = stock_code in df['StockCode'].values
        print(f"   ¿Existe '{stock_code}'? {existe}")
        ...
        if df_product.empty:
            ...
            similar = df[df['StockCode'].str.contains(stock_code, case=False, na=False)]['StockCode'].unique()
            if len(similar) > 0:
                print(f"   ⚠️  Productos similares: {list(similar[:5])}")
...
if __name__ == "__main__":
    ...
    if os.path.exists(results_path):
        print("📂 Leyendo productos desde resultados previos...")
        df_hist = pd.read_excel(results_path)
        
        # Convertir StockCode a string
        df_hist['StockCode'] = df_hist['StockCode'].astype(str).str.strip()
        
        # Filtrar buenos productos
        good_products = df_hist[
            (df_hist['Status'] == 'Éxito') & 
            (df_hist['R2'] >= 0.3)
        ].copy()
        
        # Eliminar duplicados (quedarse con mejor R²)
        good_products = good_products.sort_values('R2', ascending=False).drop_duplicates('StockCode')
        
        # Usar top 5 productos
        products_to_analyze = good_products['StockCode'].head(5).tolist()
        
        print(f"✅ Productos seleccionados (top 5 con R²≥0.3):")
        for i, p in enumerate(products_to_analyze, 1):
            r2 = good_products[good_products['StockCode'] == p]['R2'].iloc[0]
            print(f"   {i}. {p} (R²={r2:.3f})")
        
        # Verificar que existen en el dataset
        print(f"\n🔍 Verificando disponibilidad en dataset...")
        df_check = pd.read_excel(DATA_PATH, engine="openpyxl")
        df_check['StockCode'] = df_check['StockCode'].astype(str).str.strip()
        
        available_in_dataset = []
        for p in products_to_analyze:
            if p in df_check['StockCode'].values:
                count = len(df_check[df_check['StockCode'] == p])
                print(f"   ✓ {p} - {count} transacciones")
                available_in_dataset.append(p)
            else:
                print(f"   ✗ {p} - NO ENCONTRADO")
        
        if len(available_in_dataset) == 0:
            print("\n❌ Ningún producto encontrado en dataset.")
            print("📊 Mostrando productos reales en el Excel:")
            top_real = df_check['StockCode'].value_counts().head(10)
            for code, count in top_real.items():
                print(f"   {code} - {count} transacciones")
            
            # Usar productos reales
            products_to_analyze = top_real.head(3).index.tolist()
            print(f"\n⚠️  Usando estos productos reales: {products_to_analyze}")
        else:
            products_to_analyze = available_in_dataset
            print(f"\n✅ {len(products_to_analyze)} productos verificados")
            
    else:
        # Si no hay historial, usar productos más vendidos del dataset
        print("⚠️  No hay historial de entrenamientos previos")
        print("📂 Buscando productos más vendidos en dataset...")
        
        df_check = pd.read_excel(DATA_PATH, engine="openpyxl")
        df_check['StockCode'] = df_check['StockCode'].astype(str).str.strip()
        
        top_products = df_check['StockCode'].value_counts().head(5)
        products_to_analyze = top_products.index.tolist()
        
        print(f"✅ Productos seleccionados:")
        for i, (code, count) in enumerate(top_products.items(), 1):
            print(f"   {i}. {code} - {count} transacciones")
