"""
═══════════════════════════════════════════════════════════════════════════════
CUSTOMER TEMPORAL ANALYZER - Sistema Unificado de Análisis de Clientes
═══════════════════════════════════════════════════════════════════════════════

Análogo a train_all_products.py pero para análisis de clientes
Predice comportamiento de clientes en 3 horizontes temporales:
- CORTO: 4 sem → 2 sem (próxima compra inmediata)
- MEDIO: 3 meses → 1 mes (comportamiento trimestral)
- LARGO: 6 meses → 2 meses (tendencia semestral)
...
class TemporalConfig:
    """Configuración de horizontes temporales (análogo a productos)"""
    
    # Horizontes de análisis
    SHORT = {
        'name': 'short',
        'window_days': 30,      # 1 mes de historial
        'forecast_days': 7,     # Predecir próxima semana (ratio 4:1)
        'lstm_units': [64, 32],
        'epochs': 20,           # Reducido de 80 para entrenamiento más rápido
        'batch_size': 32
    }
    ...
    MEDIUM = {
        ...
        'forecast_days': 30,    # Predecir próximo mes (ratio 4:1)
        'lstm_units': [128, 64],
        'epochs': 30,           # Reducido de 100 para entrenamiento más rápido
        'batch_size': 64
    }
    ...
    LONG = {
        ...
        'forecast_days': 60,    # Predecir próximos 2 meses (ratio 4:1)
        'lstm_units': [128, 64, 32],
        'epochs': 40,           # Reducido de 120 para entrenamiento más rápido
        'batch_size': 64
    }
...
class CustomerTemporalAnalyzer:
    ...
    def __init__(self, data_path='data/online_retail.csv', output_dir='models/temporal/customer'):
        """
        Inicializa el analizador temporal de clientes
    ...
    def train_horizon_model(self, horizon_config):
        """
        ...
        metrics = {
            ...
            'total_loss': val_results[0],
            'purchase_prob_loss': val_results[1],
            'days_loss': val_results[2],
            'value_loss': val_results[3],
            'purchase_prob_accuracy': val_results[4],
            'purchase_prob_auc': val_results[5],
            'days_mae': val_results[6],
            'value_mae': val_results[7],
...
def main():
    ...
    print("╔" + "═"*68 + "╗")
    print("║" + " "*68 + "║")
    print("║" + "  CUSTOMER TEMPORAL ANALYZER - Pipeline Completo".center(68) + "║")
    ...
    if opcion == '1':
        ...
        print("\n❌ Opción inválida. Usa: python src/train/train_customer_single_horizon.py")
        return
