"""
═══════════════════════════════════════════════════════════════════════════════
CUSTOMER TEMPORAL ANALYZER V2 - Ventanas de predicción cortas
═══════════════════════════════════════════════════════════════════════════════

VERSIÓN 2: Ventanas de predicción más cortas para mejorar accuracy
- SHORT: 30 días → 7 días (igual que V1)
- MEDIUM: 120 días → 14 días (reducido de 30 días)
- LONG: 240 días → 14 días (reducido de 60 días)

Objetivo: Mantener ventanas históricas largas pero reducir forecast
para evaluar si mejora el rendimiento vs V1.
...
class TemporalConfig:
    """Configuración V2: Ventanas de predicción cortas (7/14/14 días)"""

    # Horizontes de análisis
    SHORT = {
        'name': 'short',
        'window_days': 30,      # 1 mes de historial
        'forecast_days': 7,     # Predecir próxima semana (sin cambios)
        'lstm_units': [64, 32],
        'epochs': 20,
        'batch_size': 32
    }
    ...
    MEDIUM = {
        ...
        'forecast_days': 14,    # ⭐ REDUCIDO: 14 días (era 30)
        'lstm_units': [128, 64],
        'epochs': 30,
        'batch_size': 64
    }
    ...
    LONG = {
        ...
        'forecast_days': 14,    # ⭐ REDUCIDO: 14 días (era 60)
...
class CustomerTemporalAnalyzer:
    ...
    def __init__(self, data_path='data/online_retail.csv', output_dir='models/temporal/customer_v2'):
        """
        Inicializa el analizador temporal de clientes V2

        Args:
            data_path: ruta al dataset Online Retail II
            output_dir: carpeta para guardar modelos y resultados (V2)
        """
        ...
        # Crear estructura de directorios V2
    ...
    def train_horizon_model(self, horizon_config, platform="local", script_version="v2"):
        """
        Entrena modelo para un horizonte temporal específico con MLflow tracking
        Análogo a train_product_model en productos
        """
        horizon_name = horizon_config['name']
        ...
        if MLFLOW_AVAILABLE and MLflowTracker:
            tracker = MLflowTracker(experiment_name="customers_temporal_v2", enabled=True)
            run_name = f"customers_v2_{horizon_name}_{platform}"
            ...
            tracker.start_run(
                ...
                params={
                    ...
                    "early_stopping_patience": TemporalConfig.EARLY_STOPPING_PATIENCE,
                    "reduce_lr_patience": TemporalConfig.REDUCE_LR_PATIENCE,
                    "platform": platform,
                ...
                tags={
                    "team": "PFG_LSTM",
                    "model_family": "customers_temporal",
                    "script_version": "v2",
                    "bugs_fixed": "accuracy_mae_scale"
                }
            )
            print(f"📊 MLflow tracking ID: {tracker.run_id}")
        ...
        if tracker:
            ...
            # Log modelo
            success = tracker.log_model(
                model,
                "model",
                registered_model_name=f"customers_v2_{horizon_name}"
            )
            if success:
...
def main():
    """Pipeline completo de análisis temporal de clientes"""
    import argparse

    # Parsear argumentos de línea de comandos
    parser = argparse.ArgumentParser(description='Entrenar modelos LSTM de clientes V2')
    parser.add_argument('--horizon', choices=['short', 'medium', 'long', 'all'],
                        help='Horizonte a entrenar (si no se especifica, usa menú interactivo)')
    parser.add_argument('--platform', default='local',
                        choices=['local', 'kaggle', 'colab'],
                        help='Plataforma de entrenamiento (para MLflow)')
    parser.add_argument('--script-version', default='v2_fixed',
                        help='Versión del script (para MLflow)')
    ...
    print("╔" + "═"*68 + "╗")
    print("║" + " "*68 + "║")
    print("║" + "  CUSTOMER TEMPORAL ANALYZER V2 - Pipeline Completo".center(68) + "║")
    print("║" + "  Análisis LSTM Multi-Horizonte para Clientes".center(68) + "║")
    print("║" + " "*68 + "║")
    print("╚" + "═"*68 + "╝")
    ...
    print(f"📊 Platform: {args.platform} | Script version: {args.script_version}\n")

    # Inicializar
    analyzer = CustomerTemporalAnalyzer(
        data_path='data/processed/online_retail_2.xlsx',
        output_dir='models/temporal/customer_v2'
    )
